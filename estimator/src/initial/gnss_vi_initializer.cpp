...
GNSSVIInitializer::GNSSVIInitializer(
    const std::vector<std::vector<ObsPtr>> &gnss_meas_buf_,
    const std::vector<std::vector<EphemBasePtr>> &gnss_ephem_buf_,
    const std::vector<double> &iono_params_)
    : gnss_meas_buf(gnss_meas_buf_),
      gnss_ephem_buf(gnss_ephem_buf_),
...
bool GNSSVIInitializer::coarse_localization(
  ...
  std::stringstream ss;
  for (uint32_t i = 0; i < gnss_meas_buf.size(); ++i) {
    std::copy(gnss_meas_buf[i].begin(), gnss_meas_buf[i].end(),
              std::back_inserter(accum_obs));
    std::copy(gnss_ephem_buf[i].begin(), gnss_ephem_buf[i].end(),
              std::back_inserter(accum_ephems));
  }
  for (auto it : accum_obs) {
    ss << "(time: " << std::fixed << std::setprecision(6)
       << gnss_comm::time2sec(it->time) << " id: " << it->sat << ")";
  }
  std::cout << " corrected obs: " << ss.str() << std::endl;
  Eigen::Matrix<double, 7, 1> xyzt =
      psr_pos(accum_obs, accum_ephems, iono_params);
  ...
  for (uint32_t k = 0; k < 4; ++k) {
    if (fabs(xyzt(k + 3)) < 1) xyzt(k + 3) = 0;  // not observed yet
  }

  result = xyzt;
  std::cout << " rough reference location: coarse_localization: "
            << result.transpose() << std::endl;
  return true;
}

bool GNSSVIInitializer::yaw_alignment(
    const std::vector<Eigen::Vector3d> &local_vs,
    const Eigen::Vector3d &rough_anchor_ecef, double &aligned_yaw,
    double &rcv_ddt) {
  aligned_yaw = 0;
  rcv_ddt = 0;
  ...
  while (align_iter < MAX_ITERATION && align_dx_norm > CONVERGENCE_EPSILON) {
    ...
    align_b.setZero();
    Eigen::Matrix3d align_R_enu_local(
        Eigen::AngleAxisd(est_yaw, Eigen::Vector3d::UnitZ()));
    Eigen::Matrix3d align_tmp_M;
    align_tmp_M << -sin(est_yaw), -cos(est_yaw), 0, cos(est_yaw), -sin(est_yaw),
        0, 0, 0, 0;
    ...
    for (uint32_t i = 0; i < gnss_meas_buf.size(); ++i) {
      ...
      ecef_vel_ddt.head<3>() =
          rough_R_ecef_enu * align_R_enu_local * local_vs[i];
      ecef_vel_ddt(3) = est_rcv_ddt;
      Eigen::VectorXd epoch_res;
      Eigen::MatrixXd epoch_J;
      dopp_res(ecef_vel_ddt, rough_anchor_ecef, gnss_meas_buf[i],
               all_sat_states[i], epoch_res, epoch_J);
    ...
    Eigen::VectorXd dx = -(align_G.transpose() * align_G).inverse() *
                         align_G.transpose() * align_b;
    est_yaw += dx(0);
    est_rcv_ddt += dx(1);
...
bool GNSSVIInitializer::anchor_refinement(
    const std::vector<Eigen::Vector3d> &local_ps, const double aligned_yaw,
    const double aligned_ddt, const Eigen::Matrix<double, 7, 1> &rough_ecef_dt,
    Eigen::Matrix<double, 7, 1> &refined_ecef_dt) {
  refined_ecef_dt.setZero();

  Eigen::Matrix3d aligned_R_enu_local(
      Eigen::AngleAxisd(aligned_yaw, Eigen::Vector3d::UnitZ()));
  ...
  for (uint32_t k = 0; k < 4; ++k) {
    if (rough_ecef_dt(3 + k) == 0) unobserved_sys.push_back(k);
  }
  ...
  while (refine_iter < MAX_ITERATION && refine_dx_norm > CONVERGENCE_EPSILON) {
    ...
    Eigen::Matrix3d refine_R_ecef_local =
        refine_R_ecef_enu * aligned_R_enu_local;
    for (uint32_t i = 0; i < gnss_meas_buf.size(); ++i) {
      Eigen::Matrix<double, 7, 1> ecef_xyz_dt;
      ecef_xyz_dt.head<3>() = refine_R_ecef_local * local_ps[i] + refine_anchor;
      ecef_xyz_dt.tail<4>() =
          refine_dt + aligned_ddt * i * Eigen::Vector4d::Ones();
    ...
    Eigen::VectorXd dx = -(refine_G.transpose() * refine_G).inverse() *
                         refine_G.transpose() * refine_b;
